...
class InventarioInsumo(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('fk_insumo', 'fk_sucursal', name='uq_insumo_sucursal'),
    )
...
class InventarioProducto(db.Model):
    ...
    estado             = db.Column(db.Enum('EXISTENCIA', 'AGOTADO'), nullable=False, default='EXISTENCIA')
    estatus            = db.Column(db.Enum('ACTIVO', 'INACTIVO'),    nullable=False, default='ACTIVO')
    fecha_creacion     = db.Column(db.DateTime,      nullable=False, default=datetime.datetime.now)
    usuario_creacion   = db.Column(db.Integer,       db.ForeignKey('usuario.id'), nullable=False)
    fecha_movimiento   = db.Column(db.DateTime,      nullable=False, default=datetime.datetime.now, onupdate=datetime.datetime.now)
    ...
    __table_args__ = (
        db.UniqueConstraint('fk_producto', 'fk_sucursal', name='uq_producto_sucursal'),
    )
...
class Pedido(db.Model):
    ...
    estado             = db.Column(db.Enum('ESPERANDO', 'EN PRODUCCIÓN', 'LISTO', 'ENTREGADO', 'CANCELADO'), nullable=False, default='ESPERANDO')
    estatus            = db.Column(db.Enum('ACTIVO', 'INACTIVO'), nullable=False, default='ACTIVO')
    fecha_creacion     = db.Column(db.DateTime,   nullable=False, default=datetime.datetime.now)
    usuario_creacion   = db.Column(db.Integer,    db.ForeignKey('usuario.id'), nullable=False)
    fecha_movimiento   = db.Column(db.DateTime,   nullable=False, default=datetime.datetime.now, onupdate=datetime.datetime.now)
...
class VentaDetalle(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('fk_venta', 'fk_producto', name='uq_venta_producto'),
    )
